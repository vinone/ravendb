...
namespace Raven.Database.Indexing
{
	...
	public abstract class Index : IDisposable
	{
		...
		private readonly List<Action> disposableActions = new List<Action>();
		...
		protected Index(Directory directory, string name, IndexDefinition indexDefinition)
		{
			...
			analyzer = CreateAnalyzer(disposableActions);
		...
		public void Dispose()
		{
			lock (writeLock)
			{
				...
				analyzer.Close();
				foreach (var disposableAction in disposableActions)
				{
					disposableAction();
				}
				indexWriter.Close();
		...
		protected void Write(Func<IndexWriter, bool> action)
		{
			...
			lock (writeLock)
			{
				bool shouldRecreateSearcher = action(indexWriter);
				if (shouldRecreateSearcher)
					RecreateSearcher(indexWriter.GetReader());
			}
		}
		...
		private void RecreateSearcher(IndexReader reader)
		{
			IndexSearcher _;
			using (searcher.Use(out _))
			{
		...
		private class CurrentIndexSearcher
		{
			...
			private class CleanUp : IDisposable
			{
				...
				public void Dispose()
				{
					...
					{
						parent.Searcher.GetIndexReader().Close();
